// @generated by protobuf-ts 2.9.4
// @generated from protobuf file "service.proto" (package "proto", syntax proto3)
// tslint:disable
import type { RpcTransport } from "@protobuf-ts/runtime-rpc";
import type { ServiceInfo } from "@protobuf-ts/runtime-rpc";
import { MyService } from "./service";
import type { TimeResponse } from "./service";
import type { TimeRequest } from "./service";
import type { ServerStreamingCall } from "@protobuf-ts/runtime-rpc";
import { stackIntercept } from "@protobuf-ts/runtime-rpc";
import type { HelloResponse } from "./service";
import type { HelloRequest } from "./service";
import type { UnaryCall } from "@protobuf-ts/runtime-rpc";
import type { RpcOptions } from "@protobuf-ts/runtime-rpc";
/**
 * @generated from protobuf service proto.MyService
 */
export interface IMyServiceClient {
    /**
     * @generated from protobuf rpc: SayHello(proto.HelloRequest) returns (proto.HelloResponse);
     */
    sayHello(input: HelloRequest, options?: RpcOptions): UnaryCall<HelloRequest, HelloResponse>;
    /**
     * @generated from protobuf rpc: StreamTime(proto.TimeRequest) returns (stream proto.TimeResponse);
     */
    streamTime(input: TimeRequest, options?: RpcOptions): ServerStreamingCall<TimeRequest, TimeResponse>;
}
/**
 * @generated from protobuf service proto.MyService
 */
export class MyServiceClient implements IMyServiceClient, ServiceInfo {
    typeName = MyService.typeName;
    methods = MyService.methods;
    options = MyService.options;
    constructor(private readonly _transport: RpcTransport) {
    }
    /**
     * @generated from protobuf rpc: SayHello(proto.HelloRequest) returns (proto.HelloResponse);
     */
    sayHello(input: HelloRequest, options?: RpcOptions): UnaryCall<HelloRequest, HelloResponse> {
        const method = this.methods[0], opt = this._transport.mergeOptions(options);
        return stackIntercept<HelloRequest, HelloResponse>("unary", this._transport, method, opt, input);
    }
    /**
     * @generated from protobuf rpc: StreamTime(proto.TimeRequest) returns (stream proto.TimeResponse);
     */
    streamTime(input: TimeRequest, options?: RpcOptions): ServerStreamingCall<TimeRequest, TimeResponse> {
        const method = this.methods[1], opt = this._transport.mergeOptions(options);
        return stackIntercept<TimeRequest, TimeResponse>("serverStreaming", this._transport, method, opt, input);
    }
}
